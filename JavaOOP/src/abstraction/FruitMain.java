...
public class FruitMain {
	public static void main(String[] args) {
		/*
		 ★★★ 인터페이스는 객체를 만들 수 없다.
		 FruitsInterface f = new f...(); <-----이렇게 만드는 것이 불가능함. (오로지 class만 가능하다!)
		 * */
		//FruitsInterface fruitsInterface = new FruitsInterface(); <---불가능 하지만
		/*
		 [중요문법]
		 인터페이스를 구현한 클래스의 객체생성은
		 인터페이스명(타입) 인스턴스 = new 생성자();
		 * */
		FruitsInterface apple = new Apple(); // 이건 가능
		...
		/*
		orange.count();
		인터페이스 타입(이름)으로 선언되어 생성된 객체(인스턴스)에서
		인터페이스에 선언되지 않은 메소드를
		호출할 지라도 그 메소드는 호출되지 않는다.
		이처럼 하는 이유는 기확자(아키텍쳐) 가
